...
void csv_row(double t, const std::string& phase, const VehicleState& s) {
    ...
    std::cout << std::fixed << std::setprecision(4)
        ...
        << s.yaw_rate * R2D << ","
        << s.drivetrain.throttle << "," << s.drivetrain.brake_input << ","
        << s.steer_angle * R2D << ","
...
struct Input { double throttle, brake, steer_deg; std::string phase; };

Input scenario(double t) {
    if (t < 8.0) {
    ...
    return {0, brake, steer, "trail_brake"};
}
...
int main() {
    ...
    for (double t = 0; t <= DURATION; t += DT, ++tick) {
        ...
        car.state.drivetrain.throttle = in.throttle;
        car.state.drivetrain.brake_input = in.brake;
        car.state.steer_angle = in.steer_deg * (M_PI / 180.0);

        if (tick % LOG_SKIP == 0)
            csv_row(t, in.phase, car.state);
